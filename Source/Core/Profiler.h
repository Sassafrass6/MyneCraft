...
#undef fastcall
	...
	// thread
	#define PROFILE_THREAD_START_RAW( text )   MyProfiler::threadenter( text );
	...
	#define PROFILE_THREAD_SCOPED_RAW( text )  MyProfiler::ScopedThread profiler##__LINE__ ( text );
...
#define PROFILE_THREAD_STOP()              MyProfiler::threadexit();

	// function
#define PROFILE_PAUSE()             MyProfiler::pause();
#define PROFILE_UNPAUSE()           MyProfiler::unpause();
#define PROFILE_PAUSE_SCOPED()      MyProfiler::ScopedPause profilerpause##__LINE__;

#define PROFILE_START_RAW( text )   MyProfiler::enter( text );
...
#define PROFILE_SCOPED_RAW( text )  MyProfiler::Scoped profiler##__LINE__ ( text );
...
#define PROFILE_STOP()              MyProfiler::exit();
#else
...
struct ScopedTimer {
	ScopedTimer( MyProfiler::Timer &t ) : mTimer(t) { mTimer.Start(); }
	~ScopedTimer() { mTimer.Stop(); }
protected:
	MyProfiler::Timer &mTimer;
};
